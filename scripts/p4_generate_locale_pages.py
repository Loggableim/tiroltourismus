...
def make_list_page(title, desc, emoji, name, collection, imports_extra="", hero_name=None):
    """Generate a simple list page that reads collection with locale."""
    hname = hero_name or name
    return f"""---
import BaseLayout from '../../layouts/BaseLayout.astro';
import Hero from '../../components/Hero.astro';
import Breadcrumbs from '../../components/Breadcrumbs.astro';
import SectionHeader from '../../components/SectionHeader.astro';
import {hname.title()}Card from '../../components/{hname.title()}Card.astro';
...
const data = readCollection('{collection}', locale).filter(e => isPublished(e.entry));
const mitCoords = data.filter(e => e.entry.koordinaten?.lat && e.entry.koordinaten?.lng);
---
...
<BaseLayout title="{title}" description="{desc}" locale={{locale}}>
  <main>
    ...
    <Hero title="{title}" subtitle="{desc}" emoji="{emoji}" size="md">
      <div slot="actions" class="hero-actions">
        <a href="#grid" class="btn btn-pink">Entdecken →</a>
        <span class="hero-urgency">✨ {{data.length}} Einträge</span>
      </div>
    </Hero>
    <div class="card-grid" id="grid">
      {{data.map(item => (
        <{hname.title()}Card {{...item.entry}} slug={{item.slug}} />
      ))}}
    </div>
  </main>
</BaseLayout>"""

# ── Template: Simple Detail Page ──
def make_detail_page(name, collection, nearby_default='de', redirect=f"/{collection}/", collabel_map="", extra_code=""):
...
export async function getStaticPaths() {{
  ...
  for (const locale of LANGUAGES_READY) {{
    const all = readCollection('{collection}', locale).filter(e => isPublished(e.entry));
...
if (!entry) return Astro.redirect('/{collection}/' + (locale !== 'de' ? locale + '/' : ''));
...
const nearbyEntries = entry.koordinaten ? findNearby(entry, '{collection}', locale, 8) : [];
const mapMarkers = [];
if (entry.koordinaten) {{
  mapMarkers.push({{ lat: entry.koordinaten.lat, lng: entry.koordinaten.lng, label: entry.name || entry.titel, emoji: emoji }});
}}
...
<DetailPage
  ...
  locale={{locale}}
  breadcrumbs="{{[{{ label: entry.region ? entry.region : '{name}', href: '/{collection}/' }}, {{ label: title }}]}}"
  sections="{{[
    {{ type: 'hero', title, subtitle: entry.ort || undefined, emoji, image: entry.hero_bild || (entry.bilder?.[0]?.url) }},
    {{ type: 'facts', items: [
      entry.hoehe && {{ icon: '📏', label: 'Höhe', value: entry.hoehe }},
      entry.einwohner && {{ icon: '👥', label: 'Einwohner', value: entry.einwohner }},
      ...(entry.tags ? entry.tags.slice(0,4).map(t => ({{ icon: '🏷️', label: t }})) : []),
    ].filter(Boolean) }},
...
# ── Actually let me just generate them all efficiently ──
