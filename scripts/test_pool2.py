...
print(f"Dir: {[m for m in dir(pool) if not m.startswith('_')]}")

has_creds = pool.has_credentials() if hasattr(pool, 'has_credentials') else 'N/A'
print(f"Has credentials: {has_creds}")

# Try iteration
for entry in pool:
    print(f"\nEntry type: {type(entry).__name__}")
