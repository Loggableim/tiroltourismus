#!/usr/bin/env python
"""Use Hermes internal modules to make an API call - bypassing env var issues."""
import os, sys, json

sys.path.insert(0, "E:/HermesPortable/cids-hermes-agent")
os.environ["HERMES_HOME"] = "E:/HermesPortable/home"

# First, use the credential pool properly
from agent.credential_pool import load_pool

pool = load_pool("opencode-go")
entry = pool.select()

# Get all the details
from hermes_cli.runtime_provider import resolve_runtime_provider

runtime = resolve_runtime_provider(requested="opencode-go", target_model="deepseek-v4-flash")
print(f"Runtime provider: {json.dumps({k: v if k != 'api_key' else v[:8]+'...' for k, v in runtime.items()}, indent=2)}")

# Try the API call using the runtime info
import urllib.request, urllib.error

api_key = runtime["api_key"]
base_url = runtime["base_url"]
api_mode = runtime["api_mode"]

url = f"{base_url}/chat/completions"
print(f"\nURL: {url}")
print(f"Mode: {api_mode}")
print(f"Key preview: {api_key[:12]}...")

body = {
    "model": "deepseek-v4-flash",
    "messages": [
        {"role": "system", "content": "Du schreibst kurze, sachliche Beschreibungen."},
        {"role": "user", "content": "Schreibe 2 Sätze über ein Hotel in Tirol."}
    ],
    "max_tokens": 100,
    "temperature": 0.4,
}

req = urllib.request.Request(
    url, data=json.dumps(body).encode(),
    headers={
        "Content-Type": "application/json",
        "Authorization": f"Bearer {api_key}",
    },
    method="POST"
)
try:
    resp = urllib.request.urlopen(req, timeout=30)
    result = json.loads(resp.read())
    text = result["choices"][0]["message"]["content"]
    print(f"\n✅ Success! ({len(text)} chars)")
    print(text[:200])
except urllib.error.HTTPError as e:
    print(f"\n❌ HTTP {e.code}: {e.read().decode()[:200]}")
except Exception as e:
    print(f"\n❌ {e}")
